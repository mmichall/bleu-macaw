...
class TextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path="train"):
        assert os.path.isfile(file_path)
        directory, filename = os.path.split(file_path)
        cached_features_file = os.path.join(
            directory, args.model_name_or_path + "_cached_lm_" + "_" + filename
        )
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            for line in lines:
                tokenized_text = tokenizer.convert_tokens_to_ids(tokenizer(line, padding=True, truncation=True, max_length=128))
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    logger.info("loss: %f, %d", (tr_loss - logging_loss) / args.logging_steps, global_step)
...
def main():
    ...
    parser.add_argument(
        "--train_data_file",
        type=str, required=True, help="The input training data file (a text file)."
    )
    parser.add_argument(
        "--output_dir",
        default='./output_dir',
